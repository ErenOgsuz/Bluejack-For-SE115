...
public class Game {
	...
	public static void GameRun() {
		...
		do {
			...
			System.out.println("");
			do {
				if (!playerStand && !compBust) {
					PlayerTurn();
				}
				if (!compStand && !playerBust) {
					CompTurn();
				}
		...
		System.out.println("Score: " + BlueJack.getPlayerName() + " " + Game.GetPlayerWin() + " - " + "Computer "
				+ Game.GetCompWin());
		System.out.println("");
	...
	public static void PlayerTurn() {
		...
		switch (choose) {
			...
			case 3:
				...
				if (Decks.GetPlayerHand()[choose - 1].getNumber() == 0
						&& Decks.GetPlayerHand()[choose - 1].getSpecial().equals("x2")) {
					if (pBIndex > 0) {
						Decks.GetPlayerBoard()[pBIndex - 1]
								.setNumber(Decks.GetPlayerBoard()[pBIndex - 1].getNumber() * 2);
					}
					Decks.GetPlayerBoard()[pBIndex] = Decks.GetPlayerHand()[choose - 1];
					pBIndex++;
				} else if (Decks.GetPlayerHand()[choose - 1].getNumber() == 0
						&& Decks.GetPlayerHand()[choose - 1].getSpecial().equals("+/-")) {
					if (pBIndex > 0) {
						Decks.GetPlayerBoard()[pBIndex - 1]
								.setNumber(Decks.GetPlayerBoard()[pBIndex - 1].getNumber() * -1);
					}
					Decks.GetPlayerBoard()[pBIndex] = Decks.GetPlayerHand()[choose - 1];
					pBIndex++;
				} else {
	...
	public static void CompTurn() {
		...
		switch (choose) {
			...
			case 3:
				...
				if (Decks.GetCompHand()[chooseCard].getNumber() == 0
						&& Decks.GetCompHand()[chooseCard].getSpecial().equals("x2")) {
					if (cBIndex > 0) {
					...
					cBIndex++;
				} else if (Decks.GetCompHand()[chooseCard].getNumber() == 0
						&& Decks.GetCompHand()[chooseCard].getSpecial().equals("+/-")) {
					if (cBIndex > 0) {
